    ...
    public static function returnRoutes($prefix = null) {
        ...
        if (isIndividual()):
            Route::group(array('before' => 'auth.status', 'prefix' => 'individual-listener'), function () use ($class) {
                ...
                Route::get('profile/edit', array('as' => 'individual-profile-edit',
                    'uses' => $class . '@IndividualProfileEdit'));
                Route::patch('profile/update', array('before' => 'csrf', 'as' => 'individual-profile-update',
                    'uses' => $class . '@IndividualProfileUpdate'));

                Route::get('orders', array('as' => 'individual-orders', 'uses' => $class . '@IndividualOrdersList'));
                Route::get('order/{order_id}', array('as' => 'individual-order',
                    'uses' => $class . '@IndividualOrderShow'));
                Route::delete('order/{order_id}/delete', array('as' => 'individual-order-delete',
                    'uses' => $class . '@IndividualDeleteOrder'));

                Route::get('study', array('as' => 'individual-study', 'uses' => $class . '@ListenerStudyList'));
                Route::get('study/course/{course_translite_title}', array('as' => 'individual-study-course',
                ...
                Route::get('notifications', array('as' => 'individual-notifications',
                    'uses' => $class . '@IndividualNotificationsList'));
                Route::delete('notification/{notification_id}/delete', array('as' => 'individual-notification-delete',
                    'uses' => $class . '@IndividualNotificationDelete'));
            });
        endif;
    }
        ...
        if (Request::ajax() && isIndividual()):
            ...
            if ($validator->passes()):
                if (self::IndividualAccountUpdate(Input::all())):
                    Event::fire('moderator.update-profile-individual', array(array('accountID' => 0,
                        'link' => URL::to('moderator/listeners/profile/' . Auth::user()->id),
            ...
            if ($listenerTest->result_attempt >= $success_test_percent):
                if ($test->chapter_id == 0):
                    ...
                    return Redirect::route('listener-study-test-result', array('course_translite_title' => $course_translite_title,
                        'study_test_id' => $listenerTest->id))->with('message.text', Lang::get('interface.COMPANY_LISTENER_STUDY_TEST_FINISH.success_course_test') . ' ' . round($listenerTest->result_attempt, 0) . '%</h4>')->with('message.status', 'test-result')->with('message.show_result', TRUE);
                else:
                    return Redirect::route('listener-study-test-result', array('course_translite_title' => $course_translite_title,
                        'study_test_id' => $listenerTest->id))->with('message.text', Lang::get('interface.COMPANY_LISTENER_STUDY_TEST_FINISH.success_chapter_test') . ' ' . round($listenerTest->result_attempt, 0) . '%</h4>')->with('message.status', 'test-result');
                endif;
            else:
                return Redirect::route('listener-study-test-result', array('course_translite_title' => $course_translite_title,
