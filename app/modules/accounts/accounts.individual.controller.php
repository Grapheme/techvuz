        ...
        if($validator->passes()):
            ...
            if ($listenerTest->result_attempt >= $success_test_percent):
                if ($test->chapter_id == 0):
                    ...
                    Event::fire('moderator.study.finish',array(array('accountID'=>0,'course'=>OrderListeners::where('id',$study_course_id)->first()->course()->pluck('code'),'listener'=>User_individual::where('id',Auth::user()->id)->pluck('fio'),'percent'=>$listenerTest->result_attempt,'link'=>URL::to('moderator/listeners/profile/'.Auth::user()->id))));
                    ...
                    return Redirect::route('listener-study-test-result',array('course_translite_title'=>$course_translite_title,'study_test_id'=>$listenerTest->id))->with('message.text',Lang::get('interface.COMPANY_LISTENER_STUDY_TEST_FINISH.success_chapter_test').' '.round($listenerTest->result_attempt,0).'%</h4>')->with('message.status','test-result');
                endif;
            else:
