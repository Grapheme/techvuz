...
class AccountsListenerController extends BaseController {
    ...
    public static function returnRoutes($prefix = null) {
        ...
        if (isCompanyListener()):
            Route::group(array('before' => 'auth.status.listener', 'prefix' => self::$name), function () use ($class) {
                ...
                Route::get('study/course/{course_translite_title}', array('as' => 'listener-study-course',
                    'uses' => $class . '@ListenerStudyCourse'));
                Route::post('study/course/{study_course_id}/lecture/{lecture_id}/download', array('before' => 'csrf',
                    'as' => 'listener-study-download-lecture', 'uses' => $class . '@ListenerStudyLectureDownload'));
                Route::post('study/course/{study_course_id}/lectures/download', array('before' => 'csrf',
                    'as' => 'listener-study-download-lectures', 'uses' => $class . '@ListenerStudyLecturesDownload'));
                Route::get('study/course/{course_translite_title}/test/{test_id}', array('as' => 'listener-study-testing',
                    'uses' => $class . '@ListenerStudyTesting'));
                Route::get('study/course/{course_translite_title}/test/{test_id}/confirm', array('as' => 'listener-start-study-testing',
                    'uses' => $class . '@ListenerStartStudyTesting'));
                Route::post('study/course/{course_id}/test/{test_id}/finish', array('before' => 'csrf',
                    'as' => 'listener-study-test-finish', 'uses' => $class . '@ListenerStudyTestFinish'));
                Route::get('study/course/{course_translite_title}/test/{study_test_id}/result', array('as' => 'listener-study-test-result',
                    'uses' => $class . '@ListenerStudyTestResult'));

                Route::get('notifications', array('as' => 'listener-notifications',
                    'uses' => $class . '@ListenersNotificationsList'));
                Route::delete('notification/{notification_id}/delete', array('as' => 'listener-notification-delete',
                    'uses' => $class . '@ListenerNotificationDelete'));
            });
            Route::group(array('before' => 'auth', 'prefix' => self::$name), function () use ($class) {
                Route::get('profile-approve', array('as' => 'listener-profile-approve',
                    'uses' => $class . '@ListenerProfileApprove'));
                Route::patch('profile-approve-store', array('before' => 'csrf',
                    'as' => 'listener-profile-approve-store', 'uses' => $class . '@ListenerProfileApproveStore'));
                Route::get('profile/edit', array('as' => 'listener-profile-edit',
                    'uses' => $class . '@ListenerProfileEdit'));
                Route::patch('profile/update', array('before' => 'csrf', 'as' => 'listener-profile-update',
                    'uses' => $class . '@ListenerProfileUpdate'));
            });
        endif;
    }
        ...
        if (Lectures::where('id', $lecture_id)->exists()):
            ...
            if (isset($lecture['document']['path']) && File::exists(public_path($lecture['document']['path']))):
                if ($listenerCourse->start_status == 0):
                    Event::fire('organization.study.begin', array(array('accountID' => User_listener::where('id', Auth::user()->id)->first()->organization()->pluck('id'),
    ...
    public function ListenerStudyTesting($course_translit, $test_id) {
        ...
        if ($test->chapter_id == 0):
            $test_questions_count = isset($test->course->test_questions_count) ? $test->course->test_questions_count : 0;
            if ($test_questions_count == 0):
                $test_questions_count = 10000;
            endif;
    ...
    public function ListenerStudyTestFinish($study_course_id, $test_id) {
        ...
        $test = CoursesTests::where('id', $test_id)->where('course_id', $listenerCourse->course_id)->where('active', 1)->first();
        if (!$listenerCourse || !$test || $listenerCourse->order->close_status == 1):
            return Redirect::route('listener-study');
        endif;
        $questions_ids = array();
        foreach(Input::get('questions') as $question_id => $answer_id):
            $questions_ids[] = $question_id;
        endforeach;
        if(!$questions = CoursesTestsQuestions::whereIn('id', $questions_ids)->where('test_id', $test_id)->with('answers')->get()):
            return Redirect::route('listener-study');
        endif;
            ...
            if ($listenerTest->result_attempt >= $success_test_percent):
                if ($test->chapter_id == 0):
                    ...
                    Event::fire('moderator.study.finish', array(array('accountID' => 0,
                        ...
                        'course' => OrderListeners::where('id', $study_course_id)->first()->course()->pluck('code'),
                        'percent' => $listenerTest->result_attempt)));
                    Event::fire('listener.study-finish', array(array('accountID' => Auth::user()->id,
                        'course' => OrderListeners::where('id', $study_course_id)->first()->course()->pluck('code'))));
                    AccountsOrderingController::closeOrder($listenerCourse->order_id);
                    return Redirect::route('listener-study-test-result', array('course_translite_title' => $course_translite_title,
                        'study_test_id' => $listenerTest->id))->with('message.text', Lang::get('interface.COMPANY_LISTENER_STUDY_TEST_FINISH.success_course_test') . ' ' . round($listenerTest->result_attempt) . '%</h4>')->with('message.status', 'test-result')->with('message.show_result', TRUE);
                else:
                    Event::fire('organization.study.control', array(array('accountID' => User_listener::where('id', Auth::user()->id)->first()->organization()->pluck('id'),
                        'course' => OrderListeners::where('id', $study_course_id)->first()->course()->pluck('code'),
                        'listener' => User_listener::where('id', Auth::user()->id)->pluck('fio'),
                        'percent' => $listenerTest->result_attempt)));
                    return Redirect::route('listener-study-test-result', array('course_translite_title' => $course_translite_title,
                        'study_test_id' => $listenerTest->id))->with('message.text', Lang::get('interface.COMPANY_LISTENER_STUDY_TEST_FINISH.success_chapter_test') . ' ' . round($listenerTest->result_attempt) . '%</h4>')->with('message.status', 'test-result');
                endif;
            else:
                if ($test->chapter_id == 0):
                    Event::fire('organization.study.fail-finish', array(array('accountID' => User_listener::where('id', Auth::user()->id)->first()->organization()->pluck('id'),
                        'course' => OrderListeners::where('id', $study_course_id)->first()->course()->pluck('code'),
                        'listener' => User_listener::where('id', Auth::user()->id)->pluck('fio'),
                        'percent' => $listenerTest->result_attempt)));
                else:
                    Event::fire('organization.study.fail-control', array(array('accountID' => User_listener::where('id', Auth::user()->id)->first()->organization()->pluck('id'),
                        'course' => OrderListeners::where('id', $study_course_id)->first()->course()->pluck('code'),
                        'listener' => User_listener::where('id', Auth::user()->id)->pluck('fio'),
                        'percent' => $listenerTest->result_attempt)));
                endif;
                return Redirect::route('listener-study-test-result', array('course_translite_title' => $course_translite_title,
                    'study_test_id' => $listenerTest->id))->with('message.text', Lang::get('interface.COMPANY_LISTENER_STUDY_TEST_FINISH.fail') . ' ' . round($listenerTest->result_attempt) . '%</h4>')->with('message.status', 'test-result')->with('message.show_repeat', TRUE);
            endif;
        else:
