        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 margin-bottom-25 margin-top-10">
            ...
            <div class="pull-left margin-right-10">
                <a class="btn btn-default"
                   href="{{ URL::route('modules.index',array('direction'=>$direction->id,'course'=>$course->id)) }}">Модули</a>
            </div>
            <div class="btn-group margin-top-40 margin-right-10">
                ...
                <select name="chapter_id">
                    @foreach($chapters_select as $chapter_select)
                        <option data-course="{{ $chapter_select->course_id }}"
                                value="{{ $chapter_select->id }}">{{ $chapter_select->title }}</option>
                    @endforeach
                    <option value="0">Итоговый тест</option>
                </select>
                <button type="submit" autocomplete="off" class="btn btn-success create-dublicate-test">Создать копию
                </button>
                {{ Form::close() }}
                ...
                @foreach($test->questions as $question_index => $question)
                        ...
                        <tr class="vertical-middle">
                            ...
                            <td class="col-lg-2 text-center">
                                @if(Allow::action($module['group'], 'edit'))
                                    <a href="{{ URL::route('questions.edit',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id,'question'=>$question->id)) }}"
                                       class="btn btn-success margin-right-10">Изменить</a>
                                @endif
                                @if(Allow::action($module['group'], 'delete'))
                                    <form method="DELETE"
                                          action="{{ URL::route('questions.destroy',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id,'question'=>$question->id)) }}"
                            ...
                            @foreach($question->answers as $answer_index => $answer)
                                <tr data-id="{{ $answer->id }}" class="vertical-middle">
                                    ...
                                    <td class="col-lg-2 text-center" style="white-space:nowrap;">
                                        @if(Allow::action($module['group'], 'edit'))
                                            <a href="{{ URL::route('answers.edit',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id,'question'=>$question->id,'answer'=>$answer->id)) }}"
                                               class="btn btn-success margin-right-10">Изменить</a>
                                        @endif
                                        @if(Allow::action($module['group'], 'delete'))
                                            <form method="DELETE"
                                                  action="{{ URL::route('answers.destroy',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id,'question'=>$question->id,'answer'=>$answer->id)) }}"
                                                  style="display:inline-block">
                                                <button type="submit" target="reload"
                                                        class="btn btn-danger remove-answer">Удалить
                                                </button>
                                            </form>
                                        @endif
                                    </td>
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <tr class="vertical-middle">
                            <td class="col-lg-10">
                                @if(Allow::action($module['group'], 'edit'))
                                    <a href="{{ URL::route('answers.create',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id,'question'=>$question->id)) }}"
                                       class="btn btn-success margin-right-10">Добавить ответ</a>
                                @endif
                                <a href="#" class="btn btn-success margin-right-10 js-show-answers">Показать ответы</a>
                                <a href="#" class="btn btn-success margin-right-10 js-hide-answers"
                                   style="display: none;">Скрыть ответы</a>
                            </td>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 margin-bottom-25 margin-top-10">
            <div class="pull-left margin-right-10">
                <a class="btn btn-success"
                   href="{{ URL::route('questions.create',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id)) }}">Добавить
                    вопрос</a>
            </div>
            @if(Allow::action($module['group'], 'delete'))
                <div class="pull-left margin-right-10">
                    <form method="DELETE"
                          action="{{ URL::route('testing.destroy',array('directions'=>$direction->id,'course'=>$course->id,'chapter'=>$chapter_id,'test'=>$test->id)) }}"
                          style="display:inline-block">
