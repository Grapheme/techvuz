...
<li class="orders-li js-orders-line {{ $order->payment->class }}">
    <div class="orders-li-head">
        <h4><a href="{{ URL::route('individual-order',$order->id) }}">Заказ №{{ getOrderNumber($order) }}</a></h4>
    ...
    <div class="orders-li-body">
        ...
        {{ Form::open(array('url'=>URL::route('individual-order-delete',array('order_id'=>$order->id)), 'style'=>'display:inline-block', 'method'=>'delete')) }}
            <button type="submit" class="icon-bag-btn js-delete-order" autocomplete="off" title="Удалить заказ" data-order-number="{{ getOrderNumber($order) }}"></button>
        {{ Form::close() }}
        </div>
        @endif
    </div>
