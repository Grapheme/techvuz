...
@section('content')
<main class="cabinet">
    <a class="name-dashboard" href="{{ URL::route('dashboard') }}"><h1>{{ User_organization::where('id',Auth::user()->id)->pluck('title') }}</h1></a>
...
<script>
     $(document).ready(function(){
         ...
         $('.listener-add-form input[name="email"]').keyup( function(event){
         ...
         $('.listener-add-form input[name="fio"], .listener-add-form input[name="fio_dat"], .listener-add-form input[name="position"], .listener-add-form input[name="education"]').keyup(function(event){
            var key = event.keyCode || event.charCode;
            if( key == 8 || key == 46 || key == 37 || key == 38 || key == 39 || key == 40)
                return false;
         ...
         $('.listener-add-form input[name="education"], .listener-add-form input[name="education_document_data"], .listener-add-form input[name="specialty"], .listener-add-form input[name="educational_institution"]').keyup( function(event){
            var key = event.keyCode || event.charCode;
            if( key == 8 || key == 46 || key == 37 || key == 38 || key == 39 || key == 40)
                return false;
